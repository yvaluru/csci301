...
\myreft{http://web.science.mq.edu.au/~chris/notes/second_langmach.html}
...
\li The concatenation of a string $s$ with itself $n$ times is denoted $s^n$
...
$\{a^n | n \in \mathbb{N}\}= \{\emptystring, a, aa, aaa, aaaa, \ldots\}$,
$\{ab^n | n \in \mathbb{N}\}= \{a, ab, abb, abbb, \ldots\}$,
$\{(ab)^n | n \in \mathbb{N}\}= \{\emptystring, ab, abab, ababab, \ldots\}$
...
\bi
\li $L^n = \{s_1s_2s_3\ldots s_k | k\in\mathbb{N} \wedge \forall i, s_i\in L\}$
\li If $L = \{a, bb\}$, then
